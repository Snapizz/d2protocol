var BaseMessage = require('./abstract-social-group-infos.js').class,
  util = require('util');

var AbstractSocialGroupInfos = function() {

};

util.inherits(AbstractSocialGroupInfos, BaseMessage);

AbstractSocialGroupInfos.prototype.serialize = function(output) {
  this.serializeAs_AbstractSocialGroupInfos(output);
};

AbstractSocialGroupInfos.prototype.deserialize = function(input) {
  this.deserializeAs_AbstractSocialGroupInfos(input);
};

AbstractSocialGroupInfos.prototype.serializeAs_AbstractSocialGroupInfos = function(param1) {

};

AbstractSocialGroupInfos.prototype.deserializeAs_AbstractSocialGroupInfos = function(param1) {

};

AbstractSocialGroupInfos.prototype.getTypeId = function() {
  return 416;
};

AbstractSocialGroupInfos.prototype.getClassName = function() {
  return 'AbstractSocialGroupInfos';
};

module.exports.id = 416;
module.exports.class = AbstractSocialGroupInfos;
module.exports.getInstance = function() {
  return new AbstractSocialGroupInfos();
};